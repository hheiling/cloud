# Testing of adaptive random walk Metropolis-within-Gibbs code
...
AMCMC_test = function(fit_glmmPen, dat, M, batch_length, offset){
  ...
  post_list = sample_mc_rw(fit = fit_glmmPen$fit, cov = fit_glmmPen$sigma, y = dat$y, X = dat$X,
...
fit_sim_rw = function(output, M = 10^4, batch_length = 500, offset = 5000){
...
for(i in 4:6){
  test_rw[[i]] = fit_sim_rw(test_results_noplots[[i]], M = 10000,
                            batch_length = 500, offset = 0)
}

par(mfrow = c(1,3))
for(element in 4:6){
  x = c(test_rw[[element]]$original$gibbs_accept_rate)
  y = c(test_rw[[element]]$random_walk$gibbs_accept_rate)
...
plot_test = mcmc_diagnostics(post_U = test_rw[[4]]$random_walk$post_U,
                             dat = test_rw[[4]]$dat)
